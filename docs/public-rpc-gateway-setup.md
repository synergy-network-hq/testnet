...
**Purpose:** Stand up the public-facing Testnet RPC and WebSocket endpoints on a dedicated server, proxied through nginx with TLS.

After this is done, `https://testnet-core-rpc.synergy-network.io` and `wss://testnet-core-ws.synergy-network.io` will be live and pointing at this machine.
...
synergy-testnet binary (rpc_gateway role)
...
- The `synergy-testnet-linux-amd64` binary from the repo's `binaries/` folder
...
| Name | Type | Value | TTL |
|------|------|-------|-----|
| `testnet-core-rpc` | A | `74.208.227.23` | 300 |
| `testnet-core-ws` | A | `74.208.227.23` | 300 |

Verify they resolve before proceeding:

```bash
dig +short testnet-core-rpc.synergy-network.io
dig +short testnet-core-ws.synergy-network.io
...
```
~/Library/Application Support/synergy-node-control-panel/testnet/nodes/<node-id>/
```
...
WORKSPACE_PATH="$HOME/Library/Application Support/synergy-node-control-panel/testnet/nodes/<node-id>"
SERVER="root@74.208.227.23"
BINARY="/absolute/path/to/synergy-testnet/binaries/synergy-testnet-linux-amd64"
...
# Copy binary
scp "$BINARY" $SERVER:/opt/synergy/bin/synergy-testnet
ssh $SERVER "chmod +x /opt/synergy/bin/synergy-testnet"
```
...
cp /opt/synergy/rpc-gateway/nginx.conf \
   /etc/nginx/sites-available/testnet-core-rpc.synergy-network.io.conf

ln -sf /etc/nginx/sites-available/testnet-core-rpc.synergy-network.io.conf \
...
```bash
certbot --nginx \
  -d testnet-core-rpc.synergy-network.io \
  -d testnet-core-ws.synergy-network.io \
  --agree-tos \
  --non-interactive \
...
2. Issue the certificate and save it at `/etc/letsencrypt/live/testnet-core-rpc.synergy-network.io/`
...
curl -s https://testnet-core-rpc.synergy-network.io/healthz
# Should return: ok
```
...
[Unit]
Description=Synergy Testnet RPC Gateway Node
...
ExecStart=/opt/synergy/bin/synergy-testnet start --config /opt/synergy/rpc-gateway/config/node.toml
Restart=on-failure
RestartSec=10
...
- Confirm DNS A records resolve to 74.208.227.23: `dig +short testnet-core-rpc.synergy-network.io`
