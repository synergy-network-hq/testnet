...
VALIDATE_CLOSED_SCRIPT="$ROOT_DIR/scripts/testbeta/validate-testnet-beta.sh"
...
reset_local_state() {
  ...
  while IFS= read -r machine_id; do
    [[ -z "$machine_id" ]] && continue
    local_data_dir="$ROOT_DIR/data/testbeta15/$machine_id"
...
restart_cluster() {
  ...
  echo "Starting testbeta nodes in deterministic order..."
...
post_check() {
  local rpc_url="${TESTBETA_RPC_URL:-http://127.0.0.1:5640}"
