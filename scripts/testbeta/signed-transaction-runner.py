...
for a configurable duration. Useful for load-testing testnet-beta against
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=os.path.expanduser(
            "~/Desktop/Testnet-Beta/synergy-testnet-beta/target/debug/wallet-pqc-cli"
        ),
    ...
    if not os.path.isfile(args.cli):
        if args.rebuild_cli:
            ...
            manifest = os.path.expanduser(
                "~/Desktop/Testnet-Beta/synergy-testnet-beta/src/Cargo.toml"
            )
            subprocess.run(
                [
                    "cargo",
