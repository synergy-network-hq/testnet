...
if [[ -z "${SYNERGY_LEGACY_RESET:-}" && -x "$PROJECT_DIR/scripts/testbeta/reset-testnet-beta.sh" ]]; then
    exec "$PROJECT_DIR/scripts/testbeta/reset-testnet-beta.sh" "$@"
fi
...
echo ""
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  Testbeta reset complete!${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
