...
- `networkId`: Network ID (338639 for devnet)
- `chainId`: Chain ID (338639 for devnet)
...
Get deterministic state digest data used for cross-node consensus/state checks.
...
- `state_root`: Composite deterministic state root hash
- `receipt_hash`: Transaction receipt digest
- `token_state_hash`: Token state file digest
- `validator_registry_hash`: Validator registry file digest
- `chain_state_hash`: Canonical chain-state digest
...
## SXCP (Synergy Cross-Chain Protocol)

SXCP methods expose relayer registration, quorum metadata, and attestation submission/querying.
These are devnet-facing control and verification endpoints used by relayer services and operator tooling.

### `synergy_registerRelayer`

Register (or re-register) an SXCP relayer.

**Parameters**:

- `address` (string) - Relayer address
- `public_key` (string) - Relayer ML-DSA public key (or devnet placeholder)

**Returns**: `{success: boolean, message: string, quorum: {n: number, t: number}}`

---

### `synergy_unregisterRelayer`

Deactivate an SXCP relayer (devnet convenience).

**Parameters**:

- `address` (string)

**Returns**: `{success: boolean, message?: string, error?: string, quorum?: {n: number, t: number}}`

---

### `synergy_getRelayerSet`

Get the current relayer set and quorum parameters.

**Parameters**: None

**Returns**: `{relayers: RelayerInfo[], quorum: {n: number, t: number}}`

---

### `synergy_getRelayerHealth`

Get relayer heartbeat/eligibility details used for quorum admission.

**Parameters**: None

**Returns**: `{relayers: RelayerHealth[], quorum: {n: number, t: number}}`

---

### `synergy_getSxcpStatus`

Get global SXCP status counters (relayers, events, attestations, slashing).

**Parameters**: None

**Returns**: Status object including `quorum`, `relayer_totals`, `event_totals`, and `attestation_count`

---

### `synergy_relayerHeartbeat`

Record a heartbeat for a relayer (used for basic reputation tracking).
...
Submit attestation support for a cross-chain event. The event auto-finalizes once support reaches quorum threshold `t`.

**Parameters**:

- `submitted_by` (string) - Relayer address submitting the attestation
- `event_hash` (string) - Event hash (chain-specific canonical bytes hashed)
- `aggregate_sig` (string) - Relayer signature fragment or aggregate placeholder (encoded)
- `metadata` (object, optional) - Freeform metadata (chain, tx hash, nonce, proof, etc.)

**Returns**: `{success: boolean, finalized: boolean, support_count: number, threshold: number, ...}` or error

---

### `synergy_getEventAttestation`

Get full state for one `event_hash` (supports, eligibility, finalized flag, timestamps).

**Parameters**:

- `event_hash` (string)

**Returns**: `{success: boolean, event: EventAttestationState, support_count: number, threshold: number, ...}`

---

### `synergy_getAttestations`

Fetch attestations (most recent first by default).

**Parameters**:

- `limit` (number, optional) - Max number of attestations to return (default: 100)

**Returns**: `{attestations: Attestation[], count: number}`

---

### `synergy_slashRelayer`

Slash and deactivate a relayer for malicious behavior.

**Parameters**:

- `address` (string)
- `reason` (string)
- `penalty` (number, optional; default applied when omitted)
...
Set relayer heartbeat timeout (seconds) used for online/quorum eligibility.

**Parameters**:

- `timeout_secs` (number)

**Returns**: `{success: boolean, heartbeat_timeout_secs: number, quorum: {n: number, t: number}}`

---

### `synergy_resetSxcpState`

Reset all in-memory SXCP state (devnet only).
